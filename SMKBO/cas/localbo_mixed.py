    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        ...
        with gpytorch.settings.max_cholesky_size(self.max_cholesky_size):
            ...
            gp = train_gp(
                ...
                continuous_kern_type = self.kwargs['continuous_kern_type'],
                num_mixtures1=self.kwargs['num_mixtures1'], num_mixtures2=self.kwargs['num_mixtures2']
            )
            # Save state dict
        ...
        base_kernel = gp.covar_module.base_kernel.continuous_kern

        try:  # RBF/Matern
            weights = base_kernel.lengthscale.cpu().detach().numpy().ravel()

        except:  # SMKKernel
            for sub_kernel in base_kernel.kernels:
                if hasattr(sub_kernel, 'mixture_means'):
                    mixture_means = sub_kernel.mixture_means.mean(dim=0).cpu().detach().numpy()
                    weights = (1.0 / (2 * np.pi * mixture_means)).reshape(-1)  # 确保1D输出
                    break
