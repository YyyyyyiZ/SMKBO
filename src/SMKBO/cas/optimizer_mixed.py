    ...
    def restart(self):
        ...
        if self.guided_restart and len(self.smkbo._fX):
            ...
            self.auxiliary_gp = train_gp(X_tr_torch, fX_tr_torch, False, 300,
                                         ...
                                         num_mixtures1=self.kwargs['num_mixtures1'],
                                         num_mixtures2=self.kwargs['num_mixtures2']
                                         )
            ...
            lb = np.clip(centre_cont - self.smkbo.length / 2.0, self.lb[self.n_onehot:], self.ub[self.n_onehot:])
            ub = np.clip(centre_cont + self.smkbo.length / 2.0, self.lb[self.n_onehot:], self.ub[self.n_onehot:])
            seed = np.random.randint(int(1e6))
            sobol = SobolEngine(len(self.d_cont), scramble=True, seed=seed)
            pert = sobol.draw(self.smkbo.n_init).to(dtype=torch.float32).cpu().detach().numpy()
            pert = lb + (ub - lb) * pert
            prob_perturb = min(20.0 / len(self.d_cont), 1.0)
            mask = np.random.rand(self.smkbo.n_init, len(self.d_cont)) <= prob_perturb
            ind = np.where(np.sum(mask, axis=1) == 0)[0]
            mask[ind, np.random.randint(0, len(self.d_cont) - 1, size=len(ind))] = 1
            X_init_cont = centre_cont.copy() * np.ones((self.smkbo.n_init, len(self.d_cont)))
            X_init_cont[mask] = pert[mask]

            # Generate the random samples around the categorical centre similar to the discrete CASMOPOLITAN
            X_init_cat = []
            for i in range(self.smkbo.n_init):
                cat_sample = deepcopy(
                    random_sample_within_discrete_tr_ordinal(centre_cat, self.smkbo.length_init_discrete, self.config),
                )
    ...
    def suggest(self, n_suggestions=1):
        ...
        if n_adapt > 0:
            if len(self.smkbo._X) > 0:  # Use random points if we can't fit a GP
                ...
                X[:, self.smkbo.cont_dims] = to_unit_cube(X[:, self.smkbo.cont_dims], self.lb[self.n_onehot:],
                                                          self.ub[self.n_onehot:])
                fX = copula_standardize(deepcopy(self.smkbo._fX).ravel())  # Use Copula
                next = self.smkbo._create_and_select_candidates(X, fX, length=self.smkbo.length_discrete,
                                                                       n_training_steps=300, hypers={})[-n_adapt:, :]
                next[:, self.smkbo.cont_dims] = from_unit_cube(next[:, self.smkbo.cont_dims], self.lb[self.n_onehot:],
                                                               self.ub[self.n_onehot:])
                X_next[-n_adapt:, :] = next
