...
class CauchyMixtureKernel(Kernel):
    r"""
    Computes a covariance matrix based on the Cauchy Spectral Mixture Kernel
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(CauchyMixtureKernel, self).__init__(ard_num_dims=ard_num_dims, batch_shape=batch_shape, **kwargs)
        self.num_mixtures = num_mixtures

        if mixture_scales_constraint is None:   # Scale Parameter
            mixture_scales_constraint = Positive()

        if mixture_means_constraint is None:    # Positional Parameter
    ...
    def forward(
        ...
        if not num_dims == self.ard_num_dims:
            raise RuntimeError(
                "The CauchySpectralMixtureKernel expected the input to have {} dimensionality "
